---
name: grill-me-codex
description: "Two-act plan hardening: Claude interviews the user one question at a time, then writes PLAN.md; Codex adversarially reviews it read-only and Claude revises in the same Codex session until APPROVED or MAX_ROUNDS. Require human sign-off before code. Use for /grill-me-codex, high-stakes planning, or requests to grill a plan then get a second-model review. Use /grill-with-docs-codex for docs-aware planning or /codex-review when a plan already exists. Do not use for trivial changes or existing-code review."
---
