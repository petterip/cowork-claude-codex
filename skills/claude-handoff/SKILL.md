...
Write a handoff summary of the current conversation so a fresh agent can continue the work. Resolve `RALLY_SCRIPTS="${CODEX_CLAUDE_RALLY_SCRIPTS:-$HOME/.codex/skills/codex-claude-rally/scripts}"` before launching, then run `"$RALLY_SCRIPTS/assert-subscription-auth.sh"`. If it fails, do not launch Claude or change credentials; report the blocker.
