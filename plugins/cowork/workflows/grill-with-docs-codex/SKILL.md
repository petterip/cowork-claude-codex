...
- **Act 1** is adapted from Matt Pocock's `grill-with-docs`, used under MIT (see `THIRD-PARTY-NOTICES.md`). It resolves decisions against `CONTEXT.md`/ADRs and updates them inline.
...
```markdown
# Plan: <task>
_Locked through documentation-aware planning — by Claude + <user>. Terms per CONTEXT.md._
...
Act 1 (documentation-aware planning) complete — plan locked, CONTEXT.md/ADRs updated. MAX_ROUNDS=<n>.
```
...
- **APPROVED:** present final plan + 3-bullet summary of what the two acts improved + round count. Ask: implement now — Codex builds it (`/cowork:build`), Claude builds it, or stop? No code during either act.
